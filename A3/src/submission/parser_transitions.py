class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ### START CODE HERE
        if transition == 'S':
            # if shift move first word from buffer to stack
            self.stack.append(self.buffer.pop(0))
        elif transition == 'LA':
            # if left-arc add dep to dependencies and rm second item in stack
            self.dependencies.append((self.stack[-1], self.stack.pop(-2)))
            #self.stack.pop(-2)
        elif transition == 'RA':
            # if right-arc add dep to dependencies and rm first item in stack
    ...
    def parse(self, transitions):
        ...
        """
        assert isinstance(transitions, list), "Argument for parse method should be a list."
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for idx, tran in enumerate(transitions):
            mini_batch[idx].parse_step(tran)
            if len(mini_batch[idx].stack) == 1:
