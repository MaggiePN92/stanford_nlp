...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = torch.nn.LSTM(input_size=embed_size, hidden_size=hidden_size, bidirectional=True, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### START CODE HERE (~ 8 Lines)
        X = self.model_embeddings.source(source_padded) # init embeddings
        X = nn.utils.rnn.pack_padded_sequence(X, source_lengths) # pack padded seq
        ...
        dec_init_state = (
            self.h_projection(h_cat), # h_0^{dec} (3)
            self.c_projection(c_cat)  # c_0^{dec} (4)
        )
        ### END CODE HERE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for view in torch.split(Y, 1):
            Y_t = torch.squeeze(view)
            o_prev = torch.squeeze(o_prev)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsq = torch.unsqueeze(dec_hidden, -1)
        e_t_unsq = torch.bmm(enc_hiddens_proj, dec_hidden_unsq)
        e_t = torch.squeeze(e_t_unsq, -1)
        ### END CODE HERE
        ...
        alpha_t_unsq = torch.unsqueeze(alpha_t, 1)
        a_t_unsq = torch.bmm(alpha_t_unsq, enc_hiddens)
        a_t = torch.squeeze(a_t_unsq, 1)
