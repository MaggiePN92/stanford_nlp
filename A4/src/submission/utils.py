...
def pad_sents(sents, pad_token):
    ...
    ls_of_lens = [len(sent) for sent in sents]
    longest_len = max(ls_of_lens)

    for idx, ls_sent in enumerate(sents):
        if len(ls_sent) < longest_len:
            ls_sent.extend([pad_token for i in range(longest_len - ls_of_lens[idx])])
            sents_padded.append(ls_sent)
        else:
            sents_padded.append(ls_sent)
    ### END CODE HERE
    return sents_padded
